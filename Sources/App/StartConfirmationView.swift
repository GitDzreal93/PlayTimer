    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 24) {
            ...
            VStack(alignment: .leading, spacing: 16) {
                ...
                summaryRow(icon: "timer", title: "玩耍时长", value: context.playDurationText)
                summaryRow(icon: "pause.circle.fill", title: "休息时长", value: context.breakDurationText)
            }
            .padding(18)
            .background(.regularMaterial, in: RoundedRectangle(cornerRadius: 8))

            if context.isTestModeEnabled {
                Label("测试模式已开启，这一轮会使用 \(context.playDurationText) 玩耍、\(context.breakDurationText) 休息。", systemImage: "testtube.2")
                    .font(.headline)
